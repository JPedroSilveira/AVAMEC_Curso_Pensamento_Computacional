export default {
    APRESENTACAO_CURSO: 'Apresentação do Curso',
    INTRO_PENSAMENTO_COMP: 'Introdução ao Pensamento Computacional',
    DECOMPOSICAO: 'Decomposição',
    ABSTRACAO: 'Abstração',
    RECONHECIMENTO_DE_PADROES: 'Reconhecimento de Padrões',
    ALGORITMOS: 'Algoritmos',
    PENSAMENTO_COMPUTACIONAL_E_INTERDISCIPLINARIDADE: 'Pensamento Computacional e Interdisciplinaridade',
    PENSAMENTO_COMPUTACIONAL_E_COOPERACAO: 'Pensamento Computacional e Cooperação'
}